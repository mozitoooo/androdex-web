...
function extractThreadIdsFromResult(result) {
  const ids = new Set();
  collectThreadIds(result, ids, 0, 6);
  return [...ids];
}

function collectThreadIds(value, ids, depth, maxDepth) {
  if (depth > maxDepth || !value) {
    return;
  }
  if (Array.isArray(value)) {
    for (const item of value) {
      collectThreadIds(item, ids, depth + 1, maxDepth);
    }
    return;
  }
  ...
  const directId = normalizeNonEmptyString(value?.threadId || value?.thread_id || value?.id);
  if (directId && looksLikeThreadId(directId)) {
    ids.add(directId);
  }
  const nestedThreadId = normalizeNonEmptyString(value?.thread?.id);
  if (nestedThreadId) {
    ids.add(nestedThreadId);
  }

  for (const nested of Object.values(value)) {
    collectThreadIds(nested, ids, depth + 1, maxDepth);
  }
}

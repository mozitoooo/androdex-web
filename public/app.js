...
els.openPairingBtn?.addEventListener("click", () => toggleOnboardingVisibility());
...
function renderLayout() {
  ...
  const showOnboarding = !hasSavedPairing
    || state.connectionState === "re_pair_required"
    || state.forceShowOnboarding;
  ...
  if (els.openPairingBtn) {
    els.openPairingBtn.textContent = showOnboarding ? "Hide Pairing" : "Pairing";
  }
  updateStatusBadge();
}
...
async function handleWireMessage(text) {
  ...
  // Ignore unknown wire controls to keep UI logs focused.
}
...
function processApplicationMessage(payloadText) {
  ...
  if (!parsed || typeof parsed !== "object") {
    // Ignore non-JSON host stdout/stderr noise in the chat UI.
    return;
  }
  ...
  const inferredThreadId = normalizeNonEmptyString(
    ...
    || parsed?.params?.event?.threadId
    || parsed?.params?.event?.thread_id
    || parsed?.params?.item?.threadId
    || parsed?.params?.item?.thread_id
    || parsed?.result?.thread?.id
    || parsed?.result?.threadId
    || parsed?.result?.thread_id
    || parsed?.result?.event?.threadId
    || parsed?.result?.event?.thread_id
  );
  if (inferredThreadId) {
  ...
  if (responseId && state.pendingRpc.has(responseId)) {
    ...
    if (parsed.error && typeof parsed.error === "object") {
      ...
      // Some Codex responses contain assistant output directly in the RPC result
      // instead of emitting separate notifications.
      const segments = extractTextSegments({ result: parsed.result });
      const targetThreadId = inferredThreadId || state.activeThreadId || state.threads[0]?.id || "";
      if (targetThreadId && segments.length) {
        for (const segment of segments) {
          appendMessage(targetThreadId, "assistant", segment, { appendToLast: true });
        }
      }
    }
    return;
...
async function sendPrompt() {
  ...
  try {
    ...
    if (isThreadNotFoundError(error)) {
      try {
        const fallbackThreadId = await ensureThreadId({ forceCreate: true });
        setActiveThread(fallbackThreadId);
        appendMessage(fallbackThreadId, "system", "Previous thread is unavailable in this runtime. Switched to a new thread.");
        appendMessage(fallbackThreadId, "user", prompt);
        await sendRpcRequest("turn/start", {
        ...
        els.promptInput.value = "";
        setError("Previous thread was unavailable. Continued in a new thread.");
        return;
      } catch (fallbackError) {
        setError(`Send failed: ${fallbackError.message}`);
        return;
      }
    }
...
async function hydrateThreadsFromRemote() {
  ...
  for (const candidate of methods) {
    try {
      ...
      syncThreadsFromRemote(threadIds);
...
function syncThreadsFromRemote(remoteThreadIds) {
  const uniqueRemoteIds = [...new Set(remoteThreadIds.filter((id) => looksLikeThreadId(id)))];
  if (!uniqueRemoteIds.length) {
    return;
  }

  const existingById = new Map(state.threads.map((thread) => [thread.id, thread]));
  state.threads = uniqueRemoteIds.map((id) => {
  ...
  if (!findThread(state.activeThreadId)) {
    state.activeThreadId = uniqueRemoteIds[0];
  }
...
async function startQrScanner() {
  ...
  try {
    ...
    state.scannerTimer = setInterval(() => {
      void detectQrFrame();
    }, 350);
    els.scannerHint.textContent = "Camera scanning active.";
  } catch (error) {
    const message = normalizeNonEmptyString(error?.message) || "Could not access camera.";
    setError(`Camera scan failed: ${message}`);
    els.scannerHint.textContent = "Camera failed. Use paste fallback or allow camera permission.";
  }
}
...
async function detectQrFrame() {
  if (!state.scannerRunning || !state.scannerDetector || !els.qrVideo.srcObject) {
    return;
  }
  ...
  try {
    ...
    const rawValue = normalizeNonEmptyString(results[0]?.rawValue);
    if (!rawValue) {
      return;
    }
    ...
    // frame decode errors are expected while camera is searching
  }
}
...
function extractJsonPayload(rawPayload) {
  const trimmed = normalizeNonEmptyString(rawPayload);
  if (!trimmed) {
...
function toggleOnboardingVisibility() {
  state.forceShowOnboarding = !state.forceShowOnboarding;
  renderLayout();
}

function setPairingBusy(isBusy) {
  state.pairingBusy = Boolean(isBusy);
  if (els.pairConnectBtn) {
    els.pairConnectBtn.disabled = state.pairingBusy;
    els.pairConnectBtn.textContent = state.pairingBusy ? "Pairing..." : "Pair + Connect";
  }
}
...
function extractThreadIdsFromResult(result) {
  const ids = new Set();
  addThreadIdsFromCollection(result?.threads, ids);
  addThreadIdsFromCollection(result?.result?.threads, ids);
  addThreadIdsFromCollection(result?.data?.threads, ids);
  addThreadIdsFromCollection(result?.items, ids);
  addThreadIdsFromCollection(result?.result?.items, ids);
  addThreadIdsFromCollection(result?.data?.items, ids);
  if (Array.isArray(result)) {
    addThreadIdsFromCollection(result, ids);
  }

  return [...ids];
}

function addThreadIdsFromCollection(collection, ids) {
  if (!collection) {
    return;
  }
  ...
  if (Array.isArray(collection)) {
    for (const item of collection) {
      const id = extractPotentialThreadId(item);
      if (id) {
        ids.add(id);
      }
    }
  ...
  if (typeof collection === "object") {
    for (const value of Object.values(collection)) {
      const id = extractPotentialThreadId(value);
      if (id) {
        ids.add(id);
      }
    }
  ...
  const id = extractPotentialThreadId(collection);
  if (id) {
    ids.add(id);
  }
}

function extractPotentialThreadId(item) {
...
function isDiagnosticMethod(method) {
  if (!method) {
    return false;
  }
  return [
...
function extractTextSegments(parsed) {
  const segments = [];
  collectTextFromValue(parsed?.params, segments);
  collectTextFromValue(parsed?.result, segments);
...
function collectTextFromValue(value, segments) {
  if (value == null) {
    return;
  }
  ...
  if (Array.isArray(value)) {
    for (const item of value) {
      collectTextFromValue(item, segments);
    }
    return;
  }
  ...
  collectPossibleText(segments, value.text);
  collectPossibleText(segments, value.output_text);
  collectPossibleText(segments, value.value);

  if (typeof value.delta === "string") {
    collectPossibleText(segments, value.delta);
  } else {
    collectTextFromValue(value.delta, segments);
  }

  collectTextFromValue(value.message, segments);
  collectTextFromValue(value.event, segments);
  collectTextFromValue(value.content, segments);
}
  ...
  for (const item of output) {
    ...
    if (Array.isArray(item.content)) {
      for (const part of item.content) {
        collectTextFromValue(part, segments);
      }
    }
  }
}
...
function collectPossibleText(segments, value) {
  ...
  if (isLikelyDiagnosticLine(normalized)) {
    return;
  }
  segments.push(normalized);
}

function isLikelyDiagnosticLine(text) {
  const lower = text.toLowerCase();
  if (lower.startsWith("[androdex]")) {
    return true;
  }
  if (lower.includes("relay connected") || lower.includes("socket closed")) {
    return true;
  }

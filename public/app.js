...
async function bootstrap() {
  ...
  els.threadIdInput.value = "default-thread";
...
function processApplicationMessage(payloadText) {
  ...
  const threadId =
    ...
    || "";
  if (threadId && !els.threadIdInput.value) {
...
function sendPrompt() {
  ...
  const threadId = (els.threadIdInput.value || "").trim() || "default-thread";
  sendRpc("turn/start", {
    threadId,
    prompt,
  });
...
function relayWebSocketUrl(relayUrl, sessionId) {
  ...
  return `${normalized}/relay/${sessionId}`;
}
